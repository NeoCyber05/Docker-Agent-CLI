...
Tài liệu này mô tả chi tiết sự tương quan và ánh xạ giữa các **Công cụ của Agent (Tools)**, các hàm TypeScript tương ứng trong mã nguồn, và các lệnh **Docker CLI / Docker Engine API** được thực thi dưới nền tảng của dự án **Docker Agent CLI**.
...
1. **Docker Compose CLI (`docker compose`)**: Sử dụng để quản lý vòng đời của các stack ứng dụng (khởi chạy, dừng, xem log, kiểm tra trạng thái dịch vụ trong stack). Các lệnh này được gọi thông qua lớp `BoundComposeRunner` và thực thi dưới dạng một child process.
2. **Docker Engine API (thông qua `dockerode`)**: Sử dụng để kiểm tra chi tiết trạng thái hệ thống, phát hiện sự sai lệch cấu hình (drift detection), xác thực sự tồn tại của image và pull image trực tiếp từ Docker Registry.
...
graph TD
    ...
    subgraph Tool Executions
        ...
        Tools -->|State & Drift| EngineClient[EngineClient - dockerode]
    end
...
| Tên Tool (Agent) | Danh mục | Hàm TypeScript | Docker CLI Command tương ứng | Docker Engine API Endpoint |
| :--- | :--- | :--- | :--- | :--- |
| **`plan_stack`** | `high-level` | `src/tools/planStack.ts` | *Không chạy trực tiếp lệnh thay đổi* | `GET /containers/json`<br>`GET /containers/{id}/json`<br>`GET /images/{name}/json` |
| **`apply_stack`** *(nội bộ)* | `high-level` | `src/tools/applyStack.ts` | `docker compose -p <stack> --project-directory <cwd> -f <yaml> up -d` | *Thông qua lệnh Docker Compose CLI* |
| **`destroy_stack`** | `high-level` | `src/tools/destroyStack.ts` | `docker compose -p <stack> --project-directory <cwd> -f <yaml> down [-v]` | *Thông qua lệnh Docker Compose CLI* |
| **`destroy_all_stacks`**| `high-level` | `src/tools/destroyAllStacks.ts` | Chạy tuần tự `docker compose down` cho từng stack | *Thông qua lệnh Docker Compose CLI* |
| **`get_stack_status`** | `read-only` | `src/tools/getStackStatus.ts` | `docker compose -p <stack> ps --format json`<br>`docker compose -p <stack> logs --tail <n>` | *Thông qua lệnh Docker Compose CLI* |
| **`get_logs`** | `read-only` | `src/tools/getLogs.ts` | `docker compose -p <stack> logs [--tail <n>] [--since <ts>] [<service>]` | *Thông qua lệnh Docker Compose CLI* |
| **`get_health`** | `read-only` | `src/tools/getHealth.ts` | *Không sử dụng CLI* | `GET /containers/json`<br>`GET /containers/{id}/json`<br>`GET /containers/{id}/stats` |
| **`inspect_drift`** | `read-only` | `src/tools/inspectDrift.ts` | *Không sử dụng CLI* | `GET /containers/json`<br>`GET /containers/{id}/json` |
| **`remediate_drift`** | `high-level` | `src/tools/remediateDrift.ts` | *Không chạy trực tiếp* (sinh plan → `apply_stack`) | `GET /containers/json` (qua `inspect_drift`) |
| **`pull_image`** | `escape-hatch` | `src/tools/pullImage.ts` | *Không sử dụng CLI* | `GET /images/{name}/json`<br>`POST /images/create` |
| **`exec_docker`** | `escape-hatch` | `src/tools/execDocker.ts` | `docker [ps \| inspect \| logs \| images \| network ls \| volume ls]` | *Thông qua lệnh Docker CLI* |
| **`list_stacks`** | `read-only` | `src/tools/listStacks.ts` | *Không có* | *Không có (Đọc dữ liệu lưu cục bộ)* |
| **`validate_spec`** | `read-only` | `src/tools/validateSpec.ts` | *Không có* | *Không có (Preflight)* |
| **`resolve_dependency`** | `read-only` | `src/tools/resolveDependency.ts` | *Không có* | *Không có (Preflight)* |
| **`check_port_conflict`** | `read-only` | `src/tools/checkPortConflict.ts` | *Không có* | *Không có (Preflight)* |
...
> Project cũ có thể còn thư mục `.docker-agent/stacks/` — CLI tự đổi tên sang `states/` khi khởi tạo `StateStore` nếu `states/` chưa tồn tại.
...
* **Hàm TypeScript**: `src/tools/planStack.ts`
* **Input Schema (Zod)**:
  ```typescript
  const PlanStackInputSchema = z.object({
    stackName: z.string().regex(/^[a-z][a-z0-9_-]{0,62}$/),
    intent: z.string(), // Mô tả yêu cầu bằng ngôn ngữ tự nhiên
    services: z.record(ServiceSpecSchema), // Khai báo các service (image, ports, env, volumes,...)
...
* **Xử lý nội bộ (không gọi Docker)**:
  ...
  * `injectDbHealthchecks` (`src/tools/shared/dbHealthcheck.ts`) tự động thêm healthcheck cho DB (postgres, mysql, mariadb, mongo, redis) và nâng `depends_on` lên `service_healthy`.
...
* **Hàm TypeScript**: `src/tools/applyStack.ts` *(chỉ dispatch nội bộ sau khi user duyệt `plan_ready`)*
* **Input Schema (Zod)**:
  ```typescript
  const ApplyStackInputSchema = z.object({
    stackName: z.string(),
    composeYaml: z.string(), // Chuỗi định dạng YAML hoàn chỉnh
    scaleOverrides: z.record(z.number().int().min(1)).optional(), // Cấu hình số lượng bản sao (scale)
  });
  ```
...
* **Hàm TypeScript**: `src/tools/destroyStack.ts`
* **Input Schema (Zod)**:
  ```typescript
  const DestroyStackInputSchema = z.object({
    stackName: z.string(),
    removeVolumes: z.boolean().optional(), // Có xoá volume đi kèm hay không
  });
  ```
* **Lệnh Docker CLI thực thi dưới nền**:
  ```bash
...
* **Hàm TypeScript**: `src/tools/destroyAllStacks.ts`
* **Input Schema (Zod)**:
  ```typescript
  const DestroyAllStacksInputSchema = z.object({
    removeVolumes: z.boolean().optional(),
  });
  ```
* **Lệnh Docker CLI thực thi dưới nền**:
  * Thực hiện lặp qua danh sách stacks được lấy ra từ `StateStore` và gọi tuần tự hàm `destroyStack` tương ứng với lệnh:
  ```bash
...
* **Hàm TypeScript**: `src/tools/getStackStatus.ts`
* **Input Schema (Zod)**:
  ```typescript
  const GetStackStatusInputSchema = z.object({
    stackName: z.string(),
    tailLines: z.number().int().min(0).max(1000).optional(), // Số dòng log muốn lấy (mặc định: 50)
  });
  ```
* **Lệnh Docker CLI thực thi dưới nền**:
...
* **Hàm TypeScript**: `src/tools/inspectDrift.ts`
* **Tương tác Docker Engine API**:
  * Hàm này gọi trực tiếp `detectDrift` trong `src/state/driftDetector.ts`, sử dụng thư viện `dockerode` để gọi hai endpoint API REST của Docker:
...
* **Hàm TypeScript**: `src/tools/pullImage.ts`
* **Input Schema (Zod)**:
  ```typescript
  const PullImageInputSchema = z.object({
    image: z.string(), // Ví dụ: "postgres:16-alpine"
  });
  ```
* **Tương tác Docker Engine API**:
...
* **Hàm TypeScript**: `src/tools/execDocker.ts`
...
* **Hàm TypeScript**: `src/tools/listStacks.ts`
* **Mối liên hệ với Docker**:
  * Công cụ này **không gọi trực tiếp** bất kỳ lệnh Docker CLI hay Docker Engine API nào.
  * Nó tương tác với hệ thống quản lý trạng thái cục bộ `StateStore` của Docker Agent CLI, đọc thông tin từ thư mục `.docker-agent/states/`.

---

### 3.10. `get_logs`
* **Mục đích**: Lấy snapshot log có giới hạn (tối đa 16 KiB UTF-8) để chẩn đoán lỗi — secrets được redact.
* **Hàm TypeScript**: `src/tools/getLogs.ts`
* **Input Schema (Zod)**:
  ```typescript
  const GetLogsInputSchema = z.object({
    stackName: z.string(),
    service: z.string().optional(),
    tailLines: z.number().int().min(0).max(1000).optional(), // mặc định: 100
...
* **Hàm TypeScript**: `src/tools/getHealth.ts`
* **Input Schema (Zod)**:
  ```typescript
...
```mermaid
graph TD
    Start[Bắt đầu inspect_drift] --> ReadDesired[Đọc cấu hình Desired trong StateStore]
