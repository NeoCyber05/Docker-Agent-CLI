...
from docker_agent.components.welcome_banner import build_welcome_content
...
from docker_agent.secrets.api_key_store import create_api_key_store
...
COMPACT_WELCOME_MAX_COLUMNS = 84
...
def render_welcome_banner_for_terminal(
    ...
    effective_columns = max(1, (columns or 80) - 1)
    effective_rows = rows or 24
    compact = (
        effective_rows <= COMPACT_WELCOME_MAX_ROWS
        or effective_columns < COMPACT_WELCOME_MAX_COLUMNS
    )
...
def _run_chat_session(
    ...
    *,
    ...
    sys.stdout.write(
        render_welcome_banner_for_terminal(provider=deps["provider_name"])
    )
...
def _cli_callback(
    ...
    version: Annotated[
        bool,
        typer.Option("--version", help="Show version", is_flag=True, is_eager=True),
    ] = False,
) -> None:
    if version:
