...
async def test_apply_with_rollback_success(make_loop_ctx) -> None:
    ...
    with patch(
        "docker_agent.backend.langgraph.nodes.apply_with_rollback._run_apply_tool",
...
async def test_apply_with_rollback_failure_triggers_rollback(make_loop_ctx) -> None:
    ...
    with (
        ...
        patch(
            "docker_agent.backend.langgraph.nodes.apply_with_rollback.capture_known_good",
        ) as mock_known,
        patch(
            "docker_agent.backend.langgraph.nodes.apply_with_rollback.plan_rollback",
        ) as mock_plan,
    ):
