...
async def test_plan_review_blocked_invalid_spec(make_loop_ctx, tmp_project) -> None:
    ...
    with patch(
        "docker_agent.backend.langgraph.nodes.plan_review_node._run_plan_stack",
...
async def test_plan_review_success_with_apply(make_loop_ctx, tmp_project) -> None:
    ...
    with (
        ...
        patch(
            "docker_agent.backend.langgraph.nodes.plan_review_node.interrupt",
            return_value={"kind": "approve"},
        ),
        patch(
            "docker_agent.backend.langgraph.nodes.plan_review_node.run_apply_with_rollback",
            new=AsyncMock(
