...
async def test_remediate_drift_not_remediable(make_loop_ctx, tmp_project) -> None:
    ...
    with patch(
        "docker_agent.backend.langgraph.nodes.remediate_drift_node.remediate_drift.call",
        _gen,
    ):
...
async def test_remediate_drift_success(make_loop_ctx, tmp_project) -> None:
    ...
    from docker_agent.backend.langgraph.nodes.apply_with_rollback import ApplyWithRollbackResult
    ...
    with (
        ...
        patch(
            "docker_agent.backend.langgraph.nodes.remediate_drift_node.interrupt",
            return_value={"kind": "approve"},
        ),
        patch(
            "docker_agent.backend.langgraph.nodes.remediate_drift_node.run_apply_with_rollback",
            new=AsyncMock(
