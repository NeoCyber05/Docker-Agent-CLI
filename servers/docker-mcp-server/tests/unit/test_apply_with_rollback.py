"""Tests for Docker MCP apply/rollback transactions."""
...
from docker_mcp_server.events import ToolCall, ToolResult
from docker_mcp_server.tools.apply_stack import ApplyStackResult


@pytest.mark.asyncio
async def test_apply_with_rollback_success(tmp_path: Path) -> None:
    ctx = make_ctx(tmp_path)
    events: list[object] = []
    ok_result = ApplyStackResult(ok=True, exit_code=0, yaml_path="/tmp/x.yaml")

    with patch(
        "docker_mcp_server.apply_with_rollback._run_apply_tool",
...
@pytest.mark.asyncio
async def test_apply_with_rollback_success_with_warnings(tmp_path: Path) -> None:
    ctx = make_ctx(tmp_path)
    events: list[object] = []
...
@pytest.mark.asyncio
async def test_apply_with_rollback_failure_triggers_rollback(
    tmp_path: Path,
) -> None:
    ctx = make_ctx(tmp_path)
    events: list[object] = []
    ...
    with (
        ...
        patch(
            "docker_mcp_server.apply_with_rollback.capture_known_good",
        ) as mock_known,
        patch(
            "docker_mcp_server.apply_with_rollback.plan_rollback",
        ) as mock_plan,
    ):
        from docker_mcp_server.state.rollback import KnownGood, RollbackPlan
