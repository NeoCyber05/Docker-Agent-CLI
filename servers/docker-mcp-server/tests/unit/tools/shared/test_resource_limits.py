...
from docker_mcp_server.tools.shared.resource_limits import (
...
def test_passes_for_non_privileged_port_8080() -> None:
    assert (
        check_resource_limits({"web": ServiceSpec(image="nginx:1.27-alpine", ports=["8080:80"])})
        == []
    )
...
def test_ignores_container_only_ports() -> None:
    assert (
        check_resource_limits({"web": ServiceSpec(image="nginx:1.27-alpine", ports=["80"])}) == []
    )
