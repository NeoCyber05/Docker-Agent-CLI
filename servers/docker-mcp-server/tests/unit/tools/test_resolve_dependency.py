"""Parity tests for resolve_dependency â€” mirrors resolveDependency.test.ts."""

import pytest

from docker_mcp_server.tools.resolve_dependency import ResolveDependencyInput, resolve_dependencies
...
def test_resolve_dependency_input_rejects_undeclared_network() -> None:
        ...
        "services": [
            {
                ...
                "kind": "custom",
                "image": "nginx:1.27-alpine",
                "networks": ["frontend"],  # â† 'frontend' not in declared networks
            }
        ],
        # 'networks' field intentionally omitted â†’ 'frontend' not declared
    }
    ...
    # it should fail with a clear Pydantic error â€” not a silent crash
        ...
        StackDraft.model_validate(
            {
                ...
                # No networks passed â†’ 'frontend' not declared
            }
        )
