...
from infra_agent.agent import AgentBackend, BackendQueryParams
from infra_agent.config import load_user_config
from infra_agent.core.iteration_limits import derive_recursion_limit
...
from infra_agent.mcp.client import consume_preloaded_mcp_runtime, load_mcp_langchain_tools
from infra_agent.mcp.commands import match_command
...
from infra_agent.types.permissions import permission_kind, permission_value
from infra_agent.vault.api_key_store import resolve_stored_api_key
    ...
    async def query(self, params: BackendQueryParams) -> AsyncIterator[LoopEvent]:
        ...
        async def runner() -> None:
            try:
                ...
                preloaded_runtime = consume_preloaded_mcp_runtime(params.ctx.cwd) or {}
                ...
                async def ensure_mcp_tools() -> tuple[list[Any], dict[str, Any]]:
                    ...
                    if raw_mcp_tools is None:
                        ...
                        preloaded_caps = preloaded_runtime.get("capabilities")
                        if isinstance(preloaded_caps, dict):
                            capabilities = preloaded_caps
                        else:
                ...
                async def decision_for_pending(action: dict[str, Any]) -> dict[str, Any]:
                    ...
                    if kind in {"plan_review", "action_review"}:
                        ...
                        feedback = (
                            ...
                            else getattr(response, "feedback", None)
                        )
                        return {
                            "decision": "approve"
                ...
                async def context_loader_node(_state: dict[str, Any]) -> dict[str, Any]:
                    ...
                    summary = preloaded_runtime.get("context_summary")
                    ...
                    resources = preloaded_runtime.get("resources")
                    ...
                    plugin_instructions = mcp_plugin_instructions(caps)
                ...
                async def human_approval_node(state: dict[str, Any]) -> dict[str, Any]:
                    ...
                    if decision.get("decision") != "approve":
                        ...
                        suffix = f": {feedback}" if isinstance(feedback, str) and feedback else ""
                        output = {
                            "status": "ok",
                            "result": f"denied {action.get('tool')}{suffix}",
                        }
                        if feedback:
