...
from infra_agent.commands.registry import Command, create_default_registry
...
from infra_agent.components.queue_panel import QueuePanel
...
from infra_agent.components.typed_confirm_dialog import (
...
from infra_agent.services.model_catalog import build_model_catalog, flatten_catalog
from infra_agent.services.provider_status import get_provider_statuses
from infra_agent.slash.router import SlashRouterContext, route_slash_command
...
from infra_agent.types.permissions import Approve, Deny, PermissionResponse
from infra_agent.ui.activity import ToolActivity
...
class REPL(App[None]):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._plan_feedback_request_id: str | None = None
    ...
    def on_mount(self) -> None:
        ...
        if self.resumed_record:
            from infra_agent.state.session_store import session_cwd_mismatch_warning
    ...
    def on_unmount(self) -> None:
        # Best-effort checkpoint so abrupt exits during thinking still keep session history.
        self.engine.set_activity_snapshot(self.session.activity_state.items)
        self.engine.persist_session()
    ...
    def _respond_to_plan(self, response: PermissionResponse) -> None:
        ...
        self._local_pending = None
        self._plan_feedback_request_id = None
        self._unlock_prompt_input()

    def _start_plan_feedback(self, request_id: str) -> None:
        self._local_pending = "plan_feedback"
        self._plan_feedback_request_id = request_id
        self._unlock_prompt_input()
        self.session.dispatch_activity(
            {
                "type": "assistant_text",
                "delta": (
                    "Plan bị từ chối. Hãy nhập yêu cầu chỉnh sửa plan rồi nhấn Enter "
                    "để agent re-plan."
                ),
            }
        )

    def _submit_plan_feedback(self, text: str) -> bool:
        pending = self.session.pending_event
        if (
            self._local_pending != "plan_feedback"
            or self._plan_feedback_request_id is None
            or pending is None
            or not isinstance(pending, ActionReview)
            or pending.id != self._plan_feedback_request_id
        ):
            return False
        feedback = text.strip()
        if not feedback:
            self.session.dispatch_activity(
                {
                    "type": "assistant_text",
                    "delta": "Hãy nhập lý do/yêu cầu sửa plan trước khi gửi.",
                }
            )
            return True
        self._respond_to_plan(Deny(feedback=feedback))
        return True
    ...
    def action_cancel(self) -> None:
        if self._active_log_pane is not None:
            ...
            pending = self.session.pending_event
            if pending is not None and isinstance(pending, PermissionRequest):
                from infra_agent.types.permissions import Deny
            ...
            if pending is not None and isinstance(pending, TypedConfirmRequest):
                ...
                self.session.respond(pending.id, Deny())
            self._dismiss_inline_confirm()
        elif self._local_pending == "plan_feedback":
        ...
        if self._local_pending == "permission" and event.key.lower() in ("y", "n", "a"):
            ...
            if isinstance(pending, ActionReview):
                ...
                if key == "y":
                    ...
                    self._start_plan_feedback(pending.id)
                    event.stop()
                elif key == "x":
                    self.session.dispatch_activity(
