...
export function REPL({
  ...
  // Hold a single raw-mode reference for the REPL's whole lifetime. Ink
  // reference-counts stdin raw mode by the number of mounted `useInput` hooks
  // and, when that count hits 0, removes the `'readable'` listener that
  // delivers keypresses (see ink App.handleSetRawMode). During the "thinking"
  // phase only <ThinkingIndicator/> is mounted (no `useInput`), so the count
  // would drop to 0 and tear the listener down; re-attaching it when a dialog
  // (PlanPreview, PermissionDialog, …) mounts is unreliable on some terminals,
  // leaving the dialog unable to receive keypresses. Keeping one reference held
  // here pins the count at >= 1 so the listener stays attached across every
  // streaming → dialog transition.
        ...
        switch (ev.type) {
          ...
          case "plan_ready":
            ...
            setPending({
              ...
              ...(ev.configFiles ? { configFiles: ev.configFiles } : {}),
            });
            break;
      ...
      {pending?.kind === "confirm" && (
        <PlanPreview
          ...
          {...(pending.autoGeneratedSecrets
            ...
            : {})}
          {...(pending.configFiles ? { configFiles: pending.configFiles } : {})}
          onAnswer={onAnswer}
        />
      )}
      {pending?.kind === "typed" && (
