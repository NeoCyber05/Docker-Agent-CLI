...
export class OpenRouterProvider implements Provider {
  ...
  private makeClient(apiKey: string): OpenAI {
    return new OpenAI({
      apiKey,
      baseURL: this.env.OPENROUTER_BASE_URL ?? OPENROUTER_BASE_URL,
      defaultHeaders: {
  ...
  async listModels(): Promise<string[]> {
    const apiKey = await resolveStoredApiKey("openrouter", this.env, this.apiKeyStore);
  ...
  async *stream(params: CallModelParams): AsyncGenerator<ProviderEvent> {
    const apiKey = await resolveStoredApiKey("openrouter", this.env, this.apiKeyStore);
    if (!apiKey) {
      yield { type: "error", error: new Error("OPENROUTER_API_KEY not set") };
      return;
    }
    const client = this.makeClient(apiKey);
    const model = params.model ?? this.env.OPENROUTER_MODEL ?? "openai/gpt-4o-mini";
    ...
    try {
      ...
      for await (const chunk of stream) {
        ...
        for (const call of delta.tool_calls ?? []) {
          ...
          if (!buf) {
            buf = { id: call.id ?? `or-${idx}`, name: call.function?.name ?? "", args: "" };
