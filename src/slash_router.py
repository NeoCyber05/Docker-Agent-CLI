...
from src.services.model_catalog import parse_provider_model
...
from src.state.session_store import SessionStore, format_sessions_list
...
SLASH_COMMAND_DEFS: tuple[SlashCommandDef, ...] = (
    ...
    SlashCommandDef(
        "/secrets list <stack>", "List secret keys for a stack", "/secrets list "
    ),
    SlashCommandDef(
        "/secrets rotate <stack> <service>",
            ...
            if not stack_name:
                return SlashRouteResult(
                    ...
                    effects=[
                        ...
                        {
                            "type": "emit_error",
                            "message": "Usage: /secrets list <stack>",
                        },
                    ],
                )
            result = dispatch_secrets_list(stack_name, _dispatch_ctx(ctx))
            if not result["ok"]:
                return SlashRouteResult(
                    handled=True,
                    effects=[
            ...
            if len(subparts) < 2 or not subparts[0] or not subparts[1]:
                return SlashRouteResult(
                    ...
                    effects=[
                        ...
                        {
                            "type": "emit_error",
                            "message": "Usage: /secrets rotate <stack> <service>",
                        },
                    ],
                )
            return SlashRouteResult(
                ...
                effects=[
                    {"type": "emit_user_text", "text": input_text},
                    {
                        "type": "submit_prompt",
                        "prompt": (
                            f"Rotate secrets for service {subparts[1]} "
                            f"in stack {subparts[0]}"
                        ),
                    },
                ],
            )
