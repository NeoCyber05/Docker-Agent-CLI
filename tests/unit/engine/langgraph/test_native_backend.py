...
from docker_agent.compat.docker_apply import ApplyWithRollbackResult
...
async def _run_backend(ctx, model: ToolCallingFakeModel) -> list[object]:
    ...
    mcp_env = (
        nullcontext()
        if "DOCKER_AGENT_MCP" in os.environ
        else patch.dict(os.environ, {"DOCKER_AGENT_MCP": "0"})
    )
    with (
        mcp_env,
        patch(
...
async def test_deploy_stack_approval_applies_after_preview(make_loop_ctx, tmp_project) -> None:
    ...
    with (
        ...
        patch(
            "docker_agent.compat.docker_apply.run_apply_with_rollback",
            apply,
        ),
    ):
        events = await _run_backend(ctx, model)
