...
def test_plan_ready_accepts_pydantic_auto_generated_secrets() -> None:
    from docker_agent.types.events import AutoGeneratedSecret, ConfigFileRef
    from docker_agent.types.stack import StackDiff
    ...
    state = activity_reducer(
        make_state(),
        {
            ...
            "auto_generated_secrets": [
                AutoGeneratedSecret(service="db", keys=["POSTGRES_PASSWORD"]),
            ],
            "config_files": [
                ConfigFileRef(path="./app.js", content="console.log(1)", bytes=14),
            ],
        },
    )
    plan = state.items[0]
    assert plan.type == "plan"
    assert len(plan.auto_generated_secrets) == 1
    assert plan.auto_generated_secrets[0].service == "db"
    assert plan.auto_generated_secrets[0].keys == ["POSTGRES_PASSWORD"]
    assert len(plan.config_files) == 1
    assert plan.config_files[0].path == "./app.js"
